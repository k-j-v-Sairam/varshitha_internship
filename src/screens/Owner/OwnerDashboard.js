...
import { View, StyleSheet, ScrollView, SafeAreaView, TouchableOpacity, Modal, InteractionManager } from 'react-native';
import { 
  Text, 
  Card, 
  Avatar, 
...
const OwnerDashboard = ({ navigation, route }) => {
  ...
  // --- BUG FIX: Use InteractionManager to prevent White Screen ---
  useFocusEffect(
    useCallback(() => {
      if (route.params?.openProfile) {
        // This waits for the "slide back" navigation animation to finish completely
        // before attempting to open the modal. This prevents the UI freeze/blank screen.
        const task = InteractionManager.runAfterInteractions(() => {
            setProfileVisible(true);
            // IMPORTANT: We do NOT clear the params here. 
            // Clearing them immediately causes a re-render loop.
        });
  ...
  // --- SAFE CLOSE FUNCTION ---
  // We only clear the 'openProfile' trigger when the user actually closes the modal
  // or navigates away. This keeps the state stable.
  ...
  const blockStats = [
    { label: 'Block A', status: 'Full', color: '#4CAF50' },
    { label: 'Block B', status: '2 Vacant', color: '#FF9800' },
    { label: 'Block C', status: '10 Vacant', color: '#F44336' },
  ];
  ...
  const menuItems = [
    ...
    { icon: 'bell-outline', label: 'Notifications', nav: 'Settings' },
    { icon: 'shield-check-outline', label: 'Privacy & Security', nav: 'Privacy' },
    { icon: 'help-circle-outline', label: 'Help & Support', nav: 'Help' },
  ];
    ...
    <SafeAreaView style={styles.container}>
      ...
      <Surface style={styles.header} elevation={1}>
        <TouchableOpacity 
          ...
          activeOpacity={0.7}
        >
          <Avatar.Text size={40} label="OP" style={styles.avatar} labelStyle={{lineHeight:22}} />
          <View>
            <Text variant="bodySmall" style={styles.welcomeText}>Good Morning,</Text>
            <Text variant="titleMedium" style={styles.ownerName}>Owner Dashboard</Text>
          </View>
        </TouchableOpacity>
            ...
            <View style={styles.heroContentRow}>
              ...
              <View style={styles.heroInfo}>
                ...
                <Text variant="bodySmall" style={{color: textColorDark, lineHeight: 18}}>
                  You have <Text style={{fontWeight:'bold', color: mainBrandColor}}>15% vacancy</Text> this month. Fill 12 beds to reach 100%.
                </Text>
        ...
        <View style={styles.statsRow}>
          ...
          <Card style={[styles.statCard, {backgroundColor: '#fff3e0'}]} onPress={() => navigation.navigate('Tenants')}>
             <View style={styles.statContent}>
               ...
               <Text style={[styles.statNumber, {color:'#ef6c00'}]}>5</Text>
           ...
           <Card style={[styles.statCard, {backgroundColor: '#fce4ec'}]} onPress={() => navigation.navigate('NoticeBoard')}>
             <View style={styles.statContent}>
               ...
               <Text style={[styles.statNumber, {color:'#c2185b'}]}>3</Text>
      ...
      <Modal
        ...
        onRequestClose={handleCloseProfile} // Hardware back button on Android
          ...
          <ScrollView contentContainerStyle={{padding: 20}}>
            ...
            <View style={styles.profileSection}>
              <View style={styles.profileImageContainer}>
                <Avatar.Text size={80} label="OP" style={{backgroundColor: '#E3F2FD'}} color={mainBrandColor} />
                <TouchableOpacity style={styles.editIconBadge}>
                  <Icon name="pencil" size={14} color="#FFF" />
                </TouchableOpacity>
              </View>
              <Text variant="headlineSmall" style={styles.profileName}>Sairam Owner</Text>
              <Text variant="bodyMedium" style={styles.profileEmail}>owner@hostelmanager.com</Text>
            </View>
...
const styles = StyleSheet.create({
  ...
  ownerName: { fontWeight: 'bold', fontSize: 18, color: '#333' },
  ...
  profileName: { fontWeight: 'bold', marginBottom: 5, color: '#333' },
